...
<style>
  ...
  *{box-sizing:border-box} body{margin:0;background:var(--bg);font-family:system-ui,Arial;color:var(--text)}
  ...
  .grid{display:grid;grid-template-columns:160px 1fr;gap:18px;align-items:center}
  .thumb{width:110px;height:110px;border-radius:14px;object-fit:cover;border:1px solid var(--border);background:#f3efe9}
  ...
  <div>
    ...
    <main class="content">
      <div class="card">
        ...
        <p class="subtitle">Cập nhật tên hoặc thay ảnh icon.</p>

        <form method="post" action="${ctx}/admin/category/edit" enctype="multipart/form-data" id="frm">
          ...
          <div class="grid">
            <div>
              <c:choose>
                <c:when test="${not empty category.icon}">
                  <img id="current" class="thumb" src="${fn:startsWith(category.icon,'http') ? category.icon : ctx.concat('/').concat(category.icon)}" alt="icon"/>
                </c:when>
            ...
            <div id="drop" class="drop">
              ...
              <div>
                <div style="font-weight:600">Kéo-thả ảnh mới hoặc bấm để chọn</div>
                <div class="hint">PNG/JPG, &le; 2MB</div>
              </div>
              <input id="iconFile" name="iconFile" type="file" accept="image/*" class="hidden"/>
            </div>
          </div>

          <button class="btn-main" type="submit">Cập nhật</button>
        </form>
      </div>
    </main>
  </div>
...
<script>
  ...
  function show(){
    ...
    if(f.size > 2*1024*1024) return alert('Ảnh tối đa 2MB');
